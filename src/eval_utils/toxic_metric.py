...
class ToxicHateXplain():
    def __init__(self, model_path, max_length, batch_size, use_gpu=torch.cuda.is_available()):
        ...
        self.model = Model_Rational_Label.from_pretrained(model_path)
    ...
    def scoring(self, hypo):
        ...
        for i in tqdm(range(0, len(hypo), self.batch_size)):
            with torch.no_grad():
                inputs = self.tokenizer(text=hypo[i:i + self.batch_size], return_tensors='pt', truncation=True,
                                        padding=True, max_length=self.max_length)
                if (self.use_gpu):
                    logits, _ = self.model(input_ids=inputs['input_ids'].to(device),
